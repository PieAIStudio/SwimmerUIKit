...
graph TD
    START["小邱的新 App"] 
    START --> STEP1["第 1 步：确认你的 App 已经有 React 19"]
    STEP1 --> STEP2["第 2 步：安装公开 npm 包\nnpm install @pieai/swimmer-ui-kit"]
    STEP2 --> STEP3["第 3 步：确认依赖钉住精确版本"]
...
> 帮我检查一下我的 package.json，看看我有没有 React 19；然后把 SwimmerUIKit 钉在精确版本，不要使用 `^`。

---

## 第 2 步：使用默认 npm registry

SwimmerUIKit 公开发布在 npmjs。项目不需要 scope-specific `.npmrc`、GitHub
Personal Access Token 或 package-read secret。
...
安装完成后，你的 `node_modules` 里会多出一个 `@pieai/swimmer-ui-kit` 文件夹，里面有预先构建好的组件代码、类型定义、和 CSS 文件。
...
```mermaid
graph LR
    CSS["@pieai/swimmer-ui-kit/styles.css\n（包含所有 CSS 变量 + 组件样式）"]
    ROOT["你的 App 入口文件\nmain.tsx 或 App.tsx"]
    ROOT -->|import| CSS
...
graph TD
    ...
    CHECK2 --> FIX2["路径应该是\n'@pieai/swimmer-ui-kit/styles.css'\n注意斜杠后面有 styles.css"]
```

```mermaid
graph TD
    BUG2{"安装时 npm 报错\n找不到包"}
    BUG2 --> CHECK3["检查：包名是不是 @pieai/swimmer-ui-kit？"]
    CHECK3 -->|不是| FIX3["改成官方包名并重新安装"]
    CHECK3 -->|是| CHECK4["检查 npm registry 是否为 registry.npmjs.org"]
    CHECK4 --> FIX4["移除旧的 @pieaistudio GitHub Packages 配置"]
```
